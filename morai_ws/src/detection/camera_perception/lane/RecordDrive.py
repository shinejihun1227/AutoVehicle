...
def _find_network_root():
    ...
    for _ in range(8):
        candidates.extend((
            os.path.join(cursor, "src", "common", "morai_network"),
            os.path.join(cursor, "common", "morai_network"),
            os.path.join(cursor, "morai_network"),
        ))
    ...
    for candidate in candidates:
        if candidate and os.path.isfile(
            os.path.join(candidate, "lib", "define", "Camera.py")
        ):
            return candidate
    raise SystemExit(
        "MORAI 공통 네트워크 코드를 찾지 못했습니다. MORAI_NETWORK_ROOT를 "
        "<ROI>/src/common/morai_network로 지정하세요."
    )


network_root = _find_network_root()
if network_root not in sys.path:
    sys.path.insert(0, network_root)
