...
sys.path.insert(0, os.path.join(_HERE, "..", "lane"))
from morai_camera import DEFAULT_IP, DEFAULT_PORT, CameraStream  # noqa: E402
from camera_perception.ros_lane_imu import RosImuHistory as ImuStream
...
class Runner:
    ...
    def __init__(self, args):
        ...
        self.imu = None
        if getattr(args, "imu", False) and not args.no_ros and ImuStream is not None:
            kw = {}
            if args.imu_ip:
                kw["ip"] = args.imu_ip
            if args.imu_port:
    ...
    def process(self, frame, stamp):
        ...
        if self.args.imu_attitude:
            raise ValueError('ROS integration uses yaw history only; calibrate road-relative pitch/roll separately')
...
def main(argv=None):
    ...
    cam = CameraStream(args.ip, args.port,
                       stamp_clock=(rospy.Time.now if use_ros else None)).start()
        ...
        while True:
            if use_ros:
                ...
                if rospy.is_shutdown():
                    break
            frame, meta = cam.latest_with_metadata()
            seq = meta.sequence if meta is not None else -1
            stamp = (meta.received_stamp.to_sec() if meta is not None and meta.received_stamp is not None
                     else meta.received_at if meta is not None else 0.0)
            ...
            payload["timestamp_source"] = "local_completed_frame_receipt"
            payload["all_lanes"] = [_lane_meta(c) for c in (res.lanes or [])]
