...
**현재 상태:** 이 문서를 작성한 Windows PC에는 Docker/WSL이 없어 이미지 빌드·컨테이너 실행·MORAI 주행을 수행하지 못했다. 아래는 저장소 코드와 공식 설치 문서를 대조해 만든 실행 절차이며, 현장에서 검증할 구성이다. GitHub push 완료 확인 전에는 원격 clone에 이번 코드가 있다고 가정하지 않는다.

**현재 코드는 주행 승인본이 아니다.** [미해결 사항 5개](KNOWN_ISSUES_FINAL_WS.md), 특히 P1 네 가지의 수정·회귀 검증과 Cam4 보정이 먼저다. 지금 진행할 범위는 빌드, 오프라인 검사, **제어 OFF 센서 관찰**이다. 마지막 주행 절차는 이 조건들이 해결된 후 사용한다.
...
- **미해결 코드:** P1 네 가지의 수정과 회귀 시험을 끝낸 새 코드로 이미지를 다시 빌드한다. P2 도착 순서 문제도 재현·수정한다. 현재 백업본으로는 이 조건을 충족하지 않는다.
...
시험 순서는 직선 3km/h → 적색 정지/허용 신호 출발 → 좌회전/우회전 → 방향지시등 5초 → 앞범퍼 0.5m 정지 오차 → 보행자/장애물 → 입력별 단절 정지 → 전체 경로다. 입력 단절은 별도 검증 시나리오에서만 수행하고 센서에 인위적인 noise를 추가하지 않는다. GPS blackout에서 현재 엄격 신호 융합이 정지시킬 수 있으므로 이전 곡률 단독 launch의 '계속 주행' 정책과 혼동하지 않는다.
...
- 로컬 Python unittest 252개 통과: 조작/신호 116, 카메라 42, 정지선 31, LiDAR 49, UDP bridge 14. ROS가 없는 로컬 모의 시험이며 앞의 알려진 문제를 모두 커버하지 않는다.
