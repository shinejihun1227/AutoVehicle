...
> 현재 실행 명령은 [5가지 간단 실행 안내](FIELD_TEST_CHECKLIST_20260921_KO.md)를 사용한다.
> 아래 내용은 센서 설정·보정·진단을 위한 이전 상세 절차다. `obstacle`·`merge` 프로필은 신호 정지가 없는 이전 구성이므로, 새 안내의 3·4번과 구분한다.
